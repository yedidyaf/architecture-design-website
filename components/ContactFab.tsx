...
export default function ContactFab() {
  ...
  return (
    <div ref={ref} dir="rtl" className="fixed bottom-5 left-5 z-50 flex flex-col items-start gap-3">
      <div
      ...
      >
        {CONTACT_OPTIONS.map((opt) => (
          <a
            ...
            className={`flex items-center gap-2 rounded-full py-2 pl-4 pr-3 text-sm font-medium shadow-md transition-transform hover:scale-105 ${opt.className}`}
          >
            <span className="flex h-8 w-8 shrink-0 items-center justify-center">{opt.icon}</span>
            <span>{opt.label}</span>
          </a>
        ))}
      </div>
      ...
      <button
        ...
        className="flex h-14 w-14 items-center justify-center rounded-full bg-neutral-900/90 text-white shadow-lg backdrop-blur transition-transform hover:scale-105 active:scale-95"
      >
        <svg
          viewBox="0 0 24 24"
          fill="none"
